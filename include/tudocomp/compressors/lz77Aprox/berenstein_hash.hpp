...
namespace tdc{
    ...
    private:
        ...
        const int64_t PRIME_MOD=(1ULL<<61)-1;
    ...
    public:
        ...
        long long make_hash(len_t start, len_t size,io::InputView &input_view){
            ...
            for (size_t i = 0; i < size; i++){
                hash = modulo(modulo((hash<<5)+hash) +input_view[start+i]);
            }
        ...
        inline int64_t modulo(int64_t hash){
            int64_t moded = (hash & PRIME_MOD)+(hash>>61);
        ...
        void advance_rolling_hash(rolling_hash &rhash,io::InputView &input_view){
            ...
            rhash.hashvalue = modulo(modulo((hash_minus_first<<5)+hash_minus_first)  +input_view[rhash.position+rhash.length]);
